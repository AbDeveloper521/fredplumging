/**
 * Seeds the `partnersPage` document's `sections[]` with the default Partners
 * stack from data/partnersPage.ts, VERBATIM — same order, same copy, proper
 * _key/_type per the schema. Dry run:
 *   sanity exec scripts/seed-partners-sections.ts
 * write pass (owner runs this after reading the plan):
 *   sanity exec scripts/seed-partners-sections.ts -- --confirm
 *
 * ⚠️  BEFORE RUNNING --confirm: close (or hard-reload) every open Studio tab
 * showing Partners Page. A stale tab holding an old draft that presses
 * Publish afterwards will overwrite the seeded document — this exact
 * accident happened with the About page.
 *
 * What it does, and ALL it can do:
 *  - Patches `partnersPage`, and `drafts.partnersPage` when a draft exists —
 *    same treatment, one transaction. When NO document exists at all it uses
 *    createIfNotExists({_id: "partnersPage", _type: "partnersPage"}) and
 *    then seeds it — creation, never replacement.
 *  - This page has no old fixed-field shape, so no leftover fields are
 *    expected; if a target document carries ANY unexpected field the script
 *    stops and reports rather than guessing. If a field ever needs
 *    unsetting, it is content-checked first (no image asset._ref, no
 ...
 * operations, no other document types, under any flag. (The older
 * scripts/seed-partners.ts is a different tool from an earlier phase: it
 * syncs the trustLogo/FAQ collections and siteSettings.licenseNumber. It
 * does not touch `partnersPage`, and this script does not touch anything it
 * owns.)
 *
...
const CHILD_TYPE: Record<string, string> = { items: "item", faqs: "faq" };

/**
 * The default stack as Sanity array items: JSON round-trip strips any
 * `undefined`s; nested object-array members (onboarding items, FAQ entries)
 * get deterministic _key/_type so Studio lists render and diffs stay stable.
 */
function preparedSections(): Raw[] {
  const stack = JSON.parse(JSON.stringify(defaultPartnersSections)) as Raw[];
...
async function main() {
  ...
  console.log(
    "⚠️  Close or hard-reload any open Studio tab showing Partners Page " +
  ...
  const [doc, draft] = await Promise.all([
    client.fetch<Raw | null>(`*[_id == "partnersPage"][0]`),
    client.fetch<Raw | null>(`*[_id == "drafts.partnersPage"][0]`),
  ]);

  const plans: Plan[] = [];
  if (!doc && !draft) {
    console.log(
      "No partnersPage document exists (published or draft) — the published " +
        "document will be CREATED and seeded. (Alternative without this " +
        "script: open Partners Page in /studio — it prefills with the same " +
        "stack — and press Publish.)",
    );
    plans.push({ id: "partnersPage", unset: [], create: true });
  } else {
    for (const [id, source] of [
      ["partnersPage", doc],
      ["drafts.partnersPage", draft],
    ] as const) {
      if (!source) continue;
  ...
  if (!confirm) {
    console.log(
      "\nDRY RUN — nothing written. To apply exactly the plan above:\n" +
        "  npx sanity exec scripts/seed-partners-sections.ts -- --confirm",
    );
    return;
  }
  ...
  // the only writes in the script, and they target only the partnersPage ids.
  ...
  console.log(
    `\nDone. ${plans.map((plan) => plan.id).join(" and ")} now carr${plans.length === 1 ? "ies" : "y"} ` +
      "the default Partners stack. Hard-reload /studio (Partners Page shows " +
      "the full section list, no 'Unknown fields found') and reload " +
      "/about/partners — it must render exactly what the code fallback " +
      "rendered.",
  );
}

main().catch((error) => {
  console.error("seed-partners-sections failed:", error);
  process.exit(1);
});
