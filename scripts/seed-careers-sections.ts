/**
 * Seeds the `careersPage` document's `sections[]` with the default Careers
 * stack from data/careersPage.ts, VERBATIM — the owner's three-band
 * reference (hero, values, job openings), proper _key/_type per the schema.
 * The hero photo slot stays empty (the owner uploads it in Studio). Dry run:
 *   sanity exec scripts/seed-careers-sections.ts
 * write pass (owner runs this after reading the plan):
 *   sanity exec scripts/seed-careers-sections.ts -- --confirm
 *
 * ⚠️  BEFORE RUNNING --confirm: close (or hard-reload) every open Studio tab
 * showing Careers Page. A stale tab holding an old draft that presses
 ...
 *  - Patches `careersPage`, and `drafts.careersPage` when a draft exists —
 *    same treatment, one transaction. When NO document exists at all it uses
 *    createIfNotExists({_id: "careersPage", _type: "careersPage"}) and then
 ...
 * Publish). Job content is NEVER written here — job cards stay in the
 ...
 * `undefined`s (the empty hero photo slot); nested object-array members get
 * deterministic _key/_type so Studio lists render and diffs stay stable.
 */
function preparedSections(): Raw[] {
  const stack = JSON.parse(JSON.stringify(defaultCareersSections)) as Raw[];
...
async function main() {
  ...
  console.log(
    "⚠️  Close or hard-reload any open Studio tab showing Careers Page " +
  ...
  const [doc, draft] = await Promise.all([
    client.fetch<Raw | null>(`*[_id == "careersPage"][0]`),
    client.fetch<Raw | null>(`*[_id == "drafts.careersPage"][0]`),
  ]);

  const plans: Plan[] = [];
  if (!doc && !draft) {
    console.log(
      "No careersPage document exists (published or draft) — the published " +
        "document will be CREATED and seeded. (Alternative without this " +
        "script: open Careers Page in /studio — it prefills with the same " +
        "stack — and press Publish.)",
    );
    plans.push({ id: "careersPage", unset: [], create: true });
  } else {
    for (const [id, source] of [
      ["careersPage", doc],
      ["drafts.careersPage", draft],
    ] as const) {
      if (!source) continue;
  ...
  console.log(`\nSections to seed, in order (hero photo slot stays empty):`);
  ...
  if (!confirm) {
    console.log(
      "\nDRY RUN — nothing written. To apply exactly the plan above:\n" +
        "  npx sanity exec scripts/seed-careers-sections.ts -- --confirm",
    );
    return;
  }
  ...
  // the only writes in the script, and they target only the careersPage ids.
  ...
  console.log(
    `\nDone. ${plans.map((plan) => plan.id).join(" and ")} now carr${plans.length === 1 ? "ies" : "y"} ` +
      "the default Careers stack. Hard-reload /studio (Careers Page shows " +
      "the three-section list, no 'Unknown fields found') and reload " +
      "/about/careers — it must render the owner's reference exactly.",
  );
}

main().catch((error) => {
  console.error("seed-careers-sections failed:", error);
  process.exit(1);
});
