/**
 * Seeds the `servicesIndexPage` document's `sections[]` with the default
 * /services stack from data/servicesIndexPage.ts, VERBATIM — the banner
 * (carrying the copy the old hand-built page showed) and the services grid,
 * with proper _key/_type per the schema. Dry run:
 *   sanity exec scripts/seed-services-index-sections.ts
 * write pass (owner runs this after reading the plan):
 *   sanity exec scripts/seed-services-index-sections.ts -- --confirm
 *
 * ⚠️  BEFORE RUNNING --confirm: close (or hard-reload) every open Studio tab
 * showing Services Index Page. A stale tab holding an old draft that presses
 ...
 *  - Patches `servicesIndexPage`, and `drafts.servicesIndexPage` when a
 *    draft exists — same treatment, one transaction. When NO document exists
 *    at all it uses createIfNotExists({_id: "servicesIndexPage", _type:
 *    "servicesIndexPage"}) and then seeds it — creation, never replacement.
 ...
 * Publish). Service content is NEVER written here — the service cards stay
 * in the `service` collection ("Service Pages" in Studio).
 *
...
function preparedSections(): Raw[] {
  const stack = JSON.parse(
    JSON.stringify(defaultServicesIndexSections),
  ) as Raw[];
...
async function main() {
  ...
  console.log(
    "⚠️  Close or hard-reload any open Studio tab showing Services Index Page " +
  ...
  const [doc, draft] = await Promise.all([
    client.fetch<Raw | null>(`*[_id == "servicesIndexPage"][0]`),
    client.fetch<Raw | null>(`*[_id == "drafts.servicesIndexPage"][0]`),
  ]);

  const plans: Plan[] = [];
  if (!doc && !draft) {
    console.log(
      "No servicesIndexPage document exists (published or draft) — the " +
        "published document will be CREATED and seeded. (Alternative without " +
        "this script: open Services Index Page in /studio — it prefills with " +
        "the same stack — and press Publish.)",
    );
    plans.push({ id: "servicesIndexPage", unset: [], create: true });
  } else {
    for (const [id, source] of [
      ["servicesIndexPage", doc],
      ["drafts.servicesIndexPage", draft],
    ] as const) {
      if (!source) continue;
  ...
  if (!confirm) {
    console.log(
      "\nDRY RUN — nothing written. To apply exactly the plan above:\n" +
        "  npx sanity exec scripts/seed-services-index-sections.ts -- --confirm",
    );
    return;
  }
  ...
  // the only writes in the script, and they target only the
  // servicesIndexPage ids.
  ...
  console.log(
    `\nDone. ${plans.map((plan) => plan.id).join(" and ")} now carr${plans.length === 1 ? "ies" : "y"} ` +
      "the default /services stack. Hard-reload /studio (Services Index Page " +
      "shows the two-section list, no 'Unknown fields found') and reload " +
      "/services — the banner copy and the services grid must be unchanged.",
  );
}

main().catch((error) => {
  console.error("seed-services-index-sections failed:", error);
  process.exit(1);
});
