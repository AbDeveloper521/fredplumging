/**
 * Seeds the `multifamilyIndexPage` document's `sections[]` with the default
 * /multifamily stack from data/multifamilyIndexPage.ts, VERBATIM — the banner
 * (carrying the copy the old hand-built page showed) and the property-type
 * card band, with proper _key/_type per the schema. Dry run:
 *   sanity exec scripts/seed-multifamily-index-sections.ts
 * write pass (owner runs this after reading the plan):
 *   sanity exec scripts/seed-multifamily-index-sections.ts -- --confirm
 *
 * ⚠️  BEFORE RUNNING --confirm: close (or hard-reload) every open Studio tab
 * showing Multifamily Index Page. A stale tab holding an old draft that
 ...
 *  - Patches `multifamilyIndexPage`, and `drafts.multifamilyIndexPage` when a
 *    draft exists — same treatment, one transaction. When NO document exists
 *    at all it uses createIfNotExists({_id: "multifamilyIndexPage", _type:
 *    "multifamilyIndexPage"}) and then seeds it — creation, never replacement.
 ...
 * Publish). Property-type content is NEVER written here — the cards come from
 * the `industry` collection ("Property Types" in Studio), which this script
 * does not touch.
 *
...
function preparedSections(): Raw[] {
  const stack = JSON.parse(
    JSON.stringify(defaultMultifamilyIndexSections),
  ) as Raw[];
...
async function main() {
  ...
  const [doc, draft] = await Promise.all([
    client.fetch<Raw | null>(`*[_id == "multifamilyIndexPage"][0]`),
    client.fetch<Raw | null>(`*[_id == "drafts.multifamilyIndexPage"][0]`),
  ]);

  const plans: Plan[] = [];
  if (!doc && !draft) {
    console.log(
      "No multifamilyIndexPage document exists (published or draft) — the " +
        "published document will be CREATED and seeded. (Alternative without " +
        "this script: open Multifamily Index Page in /studio — it prefills " +
        "with the same stack — and press Publish.)",
    );
    plans.push({ id: "multifamilyIndexPage", unset: [], create: true });
  } else {
    for (const [id, source] of [
      ["multifamilyIndexPage", doc],
      ["drafts.multifamilyIndexPage", draft],
    ] as const) {
      if (!source) continue;
  ...
  if (!confirm) {
    console.log(
      "\nDRY RUN — nothing written. To apply exactly the plan above:\n" +
        "  npx sanity exec scripts/seed-multifamily-index-sections.ts -- --confirm",
    );
    return;
  }
  ...
  console.log(
    `\nDone. ${plans.map((plan) => plan.id).join(" and ")} now carr${plans.length === 1 ? "ies" : "y"} ` +
      "the default /multifamily stack. Hard-reload /studio (Multifamily Index " +
      "Page shows the two-section list, no 'Unknown fields found') and reload " +
      "/multifamily — the banner copy and one card per property type must be " +
      "unchanged.",
  );
}

main().catch((error) => {
  console.error("seed-multifamily-index-sections failed:", error);
  process.exit(1);
});
